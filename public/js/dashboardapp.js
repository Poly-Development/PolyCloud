...
panel.preparePage = function(){
	if(!panel.token) return window.location = '/login.html';
...
panel.getUploads = function(album = undefined, page = undefined){
	...
	let url = 'https://polycloud.herokuapp.com/api/uploads/' + page;
	if(album !== undefined)
		url = 'https://polycloud.herokuapp.com/api/album/' + album + '/' + page;
		...
		if(panel.filesView === 'thumbs'){
			...
			for(var item of response.data.files){
				...
				if(item.thumb !== undefined)
					div.innerHTML = `<a href="${item.file}"><img src="${item.thumb}"/></a><a class="button is-small is-danger is-outlined" title="Delete file" onclick="panel.deleteFile(${item.id})"><span class="icon is-small"><i class="fa fa-trash-o"></i></span></a>`;
				else
					div.innerHTML = `<a href="${item.file}"><h1 class="title">.${item.file.split('.').pop()}</h1></a><a class="button is-small is-danger is-outlined" title="Delete file" onclick="panel.deleteFile(${item.id})"><span class="icon is-small"><i class="fa fa-trash-o"></i></span></a>`;
...
panel.deleteFile = function(id){
	swal({
		...
		function(){
			...
			axios.post('https://polycloud.herokuapp.com/api/upload/delete', {
				id: id
			})
			.then(function (response) {
...
panel.getAlbums = function(){
	...
	axios.get('https://polycloud.herokuapp.com/api/albums').then(function (response) {
		if(response.data.success === false){
			if(response.data.description === 'No token provided') return panel.verifyToken(panel.token);
		}
...
panel.renameAlbum = function(id){
	...
	swal({
		...
		axios.post('https://polycloud.herokuapp.com/api/albums/rename', {
			id: id,
			name: inputValue
		})
		.then(function (response) {
...
panel.deleteAlbum = function(id){
	swal({
		...
		function(){
			...
			axios.post('https://polycloud.herokuapp.com/api/albums/delete', {
				id: id
			})
			.then(function (response) {
...
panel.submitAlbum = function(){
	...
	axios.post('https://polycloud.herokuapp.com/api/albums', {
...
panel.getAlbumsSidebar = function(){
	...
	axios.get('https://polycloud.herokuapp.com/api/albums/sidebar')
	.then(function (response) {
		if(response.data.success === false){
			if(response.data.description === 'No token provided') return panel.verifyToken(panel.token);		}
...
panel.changeToken = function(){
	...
	axios.get('https://polycloud.herokuapp.com/api/tokens')
	.then(function (response) {
		if(response.data.success === false){
			if(response.data.description === 'No token provided') return panel.verifyToken(panel.token);
			else return swal("An error ocurred", response.data.description, "error");		
...
panel.getNewToken = function(){
	...
	axios.post('https://polycloud.herokuapp.com/api/tokens/change')
	.then(function (response) {
...
panel.sendNewPassword = function(pass){
	...
	axios.post('https://polycloud.herokuapp.com/api/password/change', {password: pass})
	.then(function (response) {
